...
class KeyResult(KiteModel):
    ...
    history = KiteAuditLog(bases=[KiteAuditInfo])
...
class EmployeeObjective(KiteModel):
    ...
    history = KiteAuditLog(bases=[KiteAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
...
class Comment(models.Model):
    ...
    history = KiteAuditLog(excluded_fields=["comment"], bases=[KiteAuditInfo])
    objects = KiteCompanyManager(
...
class QuestionTemplate(KiteModel):
...
class Question(KiteModel):
    ...
    objects = KiteCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(KiteModel):
    ...
    objects = KiteCompanyManager("question_id__template_id__company_id")


class Feedback(KiteModel):
...
class EmployeeBonusPoint(KiteModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
