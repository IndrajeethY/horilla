...
from base.kite_company_manager import KiteCompanyManager
from kite import kite_middlewares
from kite.kite_middlewares import _thread_locals
from kite.models import KiteModel, upload_path
from kite_audit.models import KiteAuditInfo, KiteAuditLog
...
class JobPosition(KiteModel):
    ...
    objects = KiteCompanyManager("department_id__company_id")
...
class JobRole(KiteModel):
    ...
    objects = KiteCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(KiteModel):
    """
    ...
    objects = KiteCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(KiteModel):
    """
...
class EmployeeShift(KiteModel):
    ...
    objects = KiteCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(KiteModel):
    """
    ...
    objects = KiteCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShiftAssign(KiteModel):
    """
...
class WorkTypeRequest(KiteModel):
    """
...
class WorkTypeRequestComment(KiteModel):
    """
...
class ShiftRequestComment(KiteModel):
    """
...
class Tags(KiteModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = KiteCompanyManager(related_company_field="company_id")
...
class DynamicEmailConfiguration(KiteModel):
    """
...
class MultipleApprovalCondition(KiteModel):
...
class AnnouncementComment(KiteModel):
    """
...
class DashboardEmployeeCharts(KiteModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(KiteModel):
...
class PenaltyAccounts(KiteModel):
    """
