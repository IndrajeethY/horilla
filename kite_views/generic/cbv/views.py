...
from kite.filters import FilterSet
from kite.group_by import group_by_queryset
from kite.kite_middlewares import _thread_locals
from kite.signals import post_generic_import, pre_generic_import
from kite_views import models
from kite_views.cbv_methods import (  # update_initial_cache,
    assign_related,
    export_xlsx,
...
from kite_views.forms import DynamicBulkUpdateForm, ToggleColumnForm
...
class KiteListView(ListView):
    ...
    def serve_import_sheet(self, request, *args, **kwargs):
        ...
        if not self.import_accessibility():
            ...
            return KiteFormView.HttpResponse()
    ...
    def get_context_data(self, **kwargs: Any):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[KiteListView] = context
        from kite.urls import path, urlpatterns
    ...
    def export_data(self, *args, **kwargs):
        ...
        class KiteListViewResorce(resources.ModelResource):
            """
        ...
        book_resource = KiteListViewResorce()
...
class KiteSectionView(TemplateView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, KiteListView)
...
@method_decorator(hx_request_required, name="dispatch")
class KiteDetailedView(DetailView):
    """
    HorillDetailedView
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, KiteDetailedView)
...
@method_decorator(hx_request_required, name="dispatch")
class KiteTabView(TemplateView):
    """
    KiteTabView
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request = request
        self.query_params = {}
        # update_initial_cache(request, CACHE, KiteTabView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[KiteTabView] = context

        return context


@method_decorator(hx_request_required, name="dispatch")
class KiteCardView(ListView):
    """
    KiteCardView
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, KiteCardView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[KiteCardView] = context
        referrer = self.request.GET.get("referrer", "")
        if referrer:
            # Remove the protocol and domain part
            referrer = "/" + "/".join(referrer.split("/")[3:])
...
class KiteFormView(FormView):
    ...
    template_name = "generic/kite_form.html"
    ids_key: str = "instance_ids"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # update_initial_cache(request, CACHE, KiteFormView)
    ...
    def get_form(self, form_class=None):
        ...
        if not hasattr(self, "form"):
            ...
            if self.request.method == "GET":
                ...
                for dynamic_tuple in self.dynamic_create_fields:
                    ...
                    form.fields[field].widget.option_template_name = (
                        "kite_widgets/select_option.html",
                    )
            ...
            # CACHE.get(self.request.session.session_key + "cbv")[KiteFormView] = form
            self.form = form
        return self.form
...
@method_decorator(hx_request_required, name="dispatch")
class KiteNavView(TemplateView):
    """
    KiteNavView
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, KiteNavView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[KiteNavView] = context
        return context
...
@method_decorator(hx_request_required, name="dispatch")
class KiteProfileView(DetailView):
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request = request
        self.ordered_ids_key = f"ordered_ids_{self.model.__name__.lower()}"
        # update_initial_cache(request, CACHE, KiteProfileView)
