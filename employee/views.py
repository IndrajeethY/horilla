...
from kite_audit.models import AccountBlockUnblock, HistoryTrackingFields
from kite_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
)
from kite_documents.models import Document, DocumentRequest
...
@login_required
@manager_can_enter("kite_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="kite_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("kite_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "kite_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "kite_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("kite_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("kite_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("kite_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("kite_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("kite_documents.add_document")
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_kite_model_class(
                    ...
                    OnboardingStage = get_kite_model_class(
                    ...
                    OnboardingTask = get_kite_model_class(
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        KITE_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_kite_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_kite_model_class(
